...
export default function ProyekCard({
  ...
  const generateTag = () => {
    ...
    mTag.forEach((e) => {
      listTag.push(
        <Link
          className={`inline-block ${e.Border} ${e.BackgroundColor} rounded-full px-3 py-2 text-sm font-semibold  mr-2 mb-2 ${e.TextColor}`}
          to={e.LinkTo}
        >
          #{e.Name.toLowerCase()}
        </Link>
  ...
  return (
    <div className="mx-3 my-4">
      <div className="max-w-xs rounded overflow-hidden shadow-lg bg-main-dark">
        ...
        <div className="px-4 pt-4 pb-2 flex">{generateTag()}</div>
      </div>
    </div>
  );
}
